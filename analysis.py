...
def get_analysis():
    ...
    try:
        ...
        c.execute("""SELECT articles.title, count(log.path) as num from log, articles where substring(log.path, 10) = articles.slug group by articles.title order by num desc limit 3""")
        ...
        c.execute("""SELECT count(log.path) as num, (select authors.name from authors where authors.id = articles.author) from log, articles where substring(log.path, 10) = articles.slug group by articles.author order by num desc;""")
        ...
        c.execute("""SELECT date, round(cast(percents as numeric), 2) from percents where percents >= 1;""")
        errors = c.fetchall()
        print(errors)
        ...
        with open("analysis.txt", "w") as text_file:
            print("Three most popular articles: \n {}".format(formatted_articles), file=text_file)
            print("Three most popular authors: \n{}".format(formatted_authors), file=text_file)
            print("Days with more that 1% errors: \n{}".format(formatted_errors), file=text_file)
